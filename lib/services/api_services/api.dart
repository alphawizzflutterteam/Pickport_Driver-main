...
class Api {
    ...
    if (res.isNotEmpty) {
      try {
        ...
        return OrderHistoryResponse(
            status: false, message: e.toString(), data: []);
      }
    } else {
      return OrderHistoryResponse(
  ...
  Future<PaymentHistoryResponse> getPaymentHistoryData(
      Map<String, String> body) async {
    if (await connectivity.checkConnectivity() == ConnectivityResult.wifi ||
        await connectivity.checkConnectivity() == ConnectivityResult.mobile) {
      String res = await _apiClient.postMethod(
          method: _apiMethods.getDriverCodEarning, body: body);
      if (res.isNotEmpty) {
        try {
  ...
  Future<ParcelHistoryResponse> getParcelHistoryData(
      Map<String, String> body) async {
    if (await connectivity.checkConnectivity() == ConnectivityResult.wifi ||
        await connectivity.checkConnectivity() == ConnectivityResult.mobile) {
      String res = await _apiClient.postMethod(
          method: _apiMethods.driverParcelHistory, body: body);
      if (res.isNotEmpty) {
        try {
  ...
  Future<NotificationResponse> getNotificationData(
    ...
    if (await connectivity.checkConnectivity() == ConnectivityResult.wifi ||
        await connectivity.checkConnectivity() == ConnectivityResult.mobile) {
      String res = await _apiClient.postMethod(
          method: _apiMethods.getNotificationList, body: body);
      if (res.isNotEmpty) {
        try {
  ...
  Future<OrderStatusUpdate> updateOrderStatusDriver(
      Map<String, String> body) async {
    if (await connectivity.checkConnectivity() == ConnectivityResult.wifi ||
        await connectivity.checkConnectivity() == ConnectivityResult.mobile) {
      String res = await _apiClient.postMethod(
          method: _apiMethods.deliverBoyOrderStatus, body: body);
      if (res.isNotEmpty) {
        try {
  ...
  Future<OrderStatusUpdate> completeOrderDriver(
      Map<String, String> body) async {
    if (await connectivity.checkConnectivity() == ConnectivityResult.wifi ||
        await connectivity.checkConnectivity() == ConnectivityResult.mobile) {
      String res = await _apiClient.postMethod(
  ...
  Future<AcceptedOrderResponse> getAcceptedOrderData(
      Map<String, String> body) async {
    if (await connectivity.checkConnectivity() == ConnectivityResult.wifi ||
        await connectivity.checkConnectivity() == ConnectivityResult.mobile) {
      String res = await _apiClient.postMethod(
          method: _apiMethods.getOrderRequest, body: body);
      if (res.isNotEmpty) {
        try {
          ...
          return AcceptedOrderResponse(
              status: 400, message: '', errorCode: 400, errorLine: 5);
        }
      } else {
        return AcceptedOrderResponse(
            status: 400, message: '', errorCode: 400, errorLine: 5);
